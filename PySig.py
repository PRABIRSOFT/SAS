...
class Signature():
    ...
    def __init__(self, data=None):
        ...
        if(data != None):
            ...
            for y in range(SignatureArea.height):
                ...
                if(bottom != 0):
                    break
            self.__signature__ = self.filteration(SignatureArea.crop((left,top,right,bottom)))
        else:
    ...
    def getSignature(self, data=None):
        img = self.__signature__
        if(img != None):
        	return img
        else:
            img = Image.open(data)
            self.__signature__ = img
            return img

    def fitTest(self, sample):
        self.__signature__ = self.__signature__.resize(sample.size)
